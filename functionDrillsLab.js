...
const nameCheck = name => {
  let result = `Cool name, ${name}`;
  if (name === 'Steven') {
    result = 'What is up Steven?';
  } else if (name === 'Bryan') {
    result = 'Hey Bryan!';
  }
  return result;
...
const faveColorFinder = color => {
  let result = 'you need to evaluate your favorite color choice';
  color = color.toLowerCase();
  if (color === 'red') {
    result = 'red is a great color';
  } else if (color === 'green') {
    result = 'green is a solid favorite color';
  } else if (color === 'black') {
    result = 'so trendy';
  }
  return result;
...
//CODE HERE
const printAllNames = arr => arr.forEach(element => {console.log(element)});
...
const bigOrSmall = arr => {
  const answers = [];
  arr.forEach(element => element > 100 ? answers.push('big') : answers.push('small'))
  return answers;
}
...
//CODE HERE
const upperCaseFunction = str => console.log(`${str.toUpperCase()}`);
...
const emailCheck = email => {
  let verify = 'must provide a valid email address';
  email = email.trim();
  if (email.includes('@')) {
...
//CODE HERE
const buyFrogs = gold => gold / 3 | 0;
const totalFrogs = buyFrogs(4);
...
const arrayIsAscending = sequentialOrder(sampleArray);
...
let bathtubScope = ["rubberDuck", "duck", "sailorDuck"]
